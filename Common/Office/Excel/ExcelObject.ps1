...
function Save-Excel{
    ...
    Begin{
        ...
        $SaveExcelIn = "$($RootPath)\ExcelReport\"+"Voyeur_Report_$($date)"
    }
    Process{
...
Function Add-Icon{
    ...
    Process{
        ...
        try{
            ...
            Write-Host "[Voyeur problem][Function Add-Icon]...$($_.Exception)" -ForegroundColor Red
            break
        }
    }
    End{
...
Function Create-Index{
    ...
    Begin{
        ...
        $CompanyLogo = "$($MyPath){0}{1}" -f "\",$ExcelSettings["CompanyLogoFront"]
        $CompanyLogoTopLeft = "$($MyPath){0}{1}" -f "\",$ExcelSettings["CompanyUserTopLeft"]
    ...
    Process{
        try{
            ...
            foreach ($Sheet in $WorkBook.WorkSheets){
                ...
                $v = $WorkSheet.Hyperlinks.Add($WorkSheet.Cells.Item($row,$col),"","'$($Sheet.Name)'"+"!$($r)","",$Sheet.Name)
				$row++
            }
            ...
            Write-Host "[Voyeur problem][Function Create-Index]...$($_.Exception)" -ForegroundColor Red
        }
        ...
        #Add AuditorName
        $WorkSheet.Cells.Item(5,1).FormulaLocal = $AuditorName
...
Function Create-About{
    ...
    Begin{
        ...
        $cnt = 1
        foreach ($webpage in $ExcelSettings["HyperLinkcompanyName"]){
    ...
    Process{
        ...
        $CompanyLogo = "$($MyPath){0}{1}" -f "\",$ExcelSettings["CompanyLogo"]
        #Image format and properties        
        $LinkToFile = $msoFalse
        $SaveWithDocument = $msoTrue
        $Left = 400
