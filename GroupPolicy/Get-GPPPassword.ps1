...
#>
    ...
    {
        Write-Host 'Machine is not joined to a domain. Trying to download XML files...' -ForegroundColor Yellow
		#discover potential files containing passwords ; not complaining in case of denied access to a directory. Added Drives.xml
    	$XMlFiles = Get-ChildItem -Path "\\$NetBios\SYSVOL" -Recurse -ErrorAction SilentlyContinue -Include 'Groups.xml','Services.xml','Scheduledtasks.xml','DataSources.xml','Drives.xml'			
	}
	else
		{
			#discover potential files containing passwords ; not complaining in case of denied access to a directory
    		$XMlFiles = Get-ChildItem -Path "\\$Env:USERDNSDOMAIN\SYSVOL" -Recurse -ErrorAction SilentlyContinue -Include 'Groups.xml','Services.xml','Scheduledtasks.xml','DataSources.xml','Drives.xml'
		}
    ...
    if ( -not $XMlFiles )
    {
        Write-Host 'No files containing encrypted passwords found.' -ForegroundColor Magenta
    }
	$GPPLeaks = @()
